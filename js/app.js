...
const amuntInputElem = $.querySelector("#amount");
const generatElembtn = $.querySelector("button");
const resultElem = $.querySelector("#result");

generatElembtn.addEventListener("click", function (event) {
  event.preventDefault();

  let amuntElemValue = amuntInputElem.value;
  let randomTextIndex = Math.floor(Math.random() * dummyText.length);
  if (
    amuntElemValue < 0 ||
    amuntElemValue > 9 ||
    amuntElemValue === "" ||
    isNaN(amuntElemValue)
  ) {
    resultElem.innerHTML = dummyText[randomTextIndex]
  } else {
    let slicedArray = dummyText.slice( 0, amuntElemValue)
    let finalTextArray = slicedArray.map( function (item){
      return "<p>" + item+"</p>"
    }).join("")
